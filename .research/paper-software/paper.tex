...
\texttt{FockMap} now includes a bosonic canonical commutation algebra
for symbolic normal ordering of ladder-operator expressions.  This
extends the same typed expression pipeline to bosonic sectors and
supports mixed-statistics model assembly in a single representation.
...
xUnit + FsCheck test suite (427 passing tests), including both
property-based algebraic checks and targeted edge-case regressions.
Current coverage is 96.51\% line and 86.94\% branch on the core library.
...
normal-ordering support before any downstream qubit mapping or
truncation strategy is applied.
...
cross-encoding consistency checks.  At submission time, 427 tests pass
for the software package, with 96.51\% line coverage and 86.94\% branch
...
These checks provide evidence that symbolic rewriting is robust in both
fermionic and bosonic workflows, including malformed parser inputs,
non-canonical operator orderings, and same-index rewrite edge cases.
...
FockMap additionally provides symbolic bosonic normal ordering.}
